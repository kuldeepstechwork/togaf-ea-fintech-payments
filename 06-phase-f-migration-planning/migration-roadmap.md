...
## Cutover Mechanism and Rollback Plan

Every "Cutover" step in the Gantt above is a single, specific mechanical event, not a milestone label: the orchestration hub's Routing Engine (see [`04-phase-d-technology-architecture/reference-architecture.md`](../04-phase-d-technology-architecture/reference-architecture.md)) holds a per-rail routing configuration state, `legacy` or `migrated`. Cutover is that state flipping to `migrated` for the rail in question; rollback is the same flag flipping back. Because this is a configuration change enforced by the Routing Engine rather than a code deployment, both directions are executable in minutes, not on a redeploy cycle.

**Pre-cutover (dual-run / shadow mode):** For the wave's dual-run window, live traffic for the rail is routed end-to-end through the legacy integration — the legacy path remains the caller-visible result — while a shadow copy of the same traffic is simultaneously routed through the new adapter, transformation layer, and ledger, with outputs compared transaction-by-transaction. This is dark-launch, not A/B: the new path's output during dual-run is never caller-visible and never overrides the legacy outcome, so a defect found during dual-run has zero merchant or customer impact.

**Cutover:** Once the wave's exit criteria are met and the ARB go/no-go checkpoint passes, the Routing Engine flag flips to `migrated`: the new adapter becomes the live path, and the legacy integration stops receiving new transactions for that rail (transactions already in flight on the legacy path are drained to completion rather than aborted mid-transaction).

**Rollback triggers and procedure:** A rollback is triggered by any of (a) a post-cutover reconciliation discrepancy exceeding the rail's defined tolerance, (b) a Tier-0 SLA breach attributable to the new adapter or hub path, or (c) an ARB-declared incident severity warranting reversion. Rollback is the flag flip in reverse, executed by Platform Engineering on-call without a code deployment. Ledger entries the orchestration core wrote for that rail between cutover and rollback are not deleted or mutated — per [ADR-002](../adrs/adr-002-single-ledger-of-truth-design.md) the ledger is append-only — so the reconciliation bridge (see [`transition-architectures.md`](transition-architectures.md)) treats the rolled-back window as a standing reconciliation exception: it matches those ledger entries against the legacy system's independently recorded outcome for the same transactions and posts adjustment entries for any mismatch before Finance Operations' next close, rather than the migrated rail's ledger data being discarded. A rolled-back rail re-enters dual-run rather than attempting a second immediate cutover, so the triggering defect is verified fixed under shadow traffic before cutover is retried.

This mechanism is identical for every wave; only the traffic volume behind it and the wave-specific tolerance thresholds in the exit criteria change.
